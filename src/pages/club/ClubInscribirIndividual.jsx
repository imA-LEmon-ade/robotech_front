...
export default function ClubInscribirIndividual({ categoria }) {
  ...
  const inscribir = async () => {
    ...
    if (!robotSeleccionado) {
      Swal.fire("Atención", "Selecciona un robot", "warning");
      return;
    }
    ...
    try {
      ...
      Swal.fire(
        ...
        "El robot quedó en estado pendiente",
        "success"
      );
      ...
      Swal.fire(
        "Error",
        err?.response?.data ?? "No se pudo inscribir el robot",
        "error"
      );
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="card shadow-sm p-4">
      ...
      <h5 className="fw-bold mb-3">
        Inscripción Individual – {categoria.categoria}
      </h5>
