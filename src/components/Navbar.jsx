...
import "../styles/navbar.css"; // Descomenta esta línea si guardaste el CSS en un archivo aparte

export default function Navbar() {

  const { user, rol, logout } = useContext(AuthContext);

  // Función helper: Si la ruta coincide, React Router pone isActive = true.
  ...
  return (
    ...
    <nav className="navbar navbar-expand-lg robotech-navbar sticky-top">
      <div className="container">
        ...
        {/* Logo: Usamos Link para evitar recarga de página */}
        <Link className="navbar-brand" to="/">
          <img 
            src="/img/logo.jpg" 
            alt="Logo Robotech" 
            height="50" 
            className="d-inline-block align-text-top rounded" // Clases extra para mejor alineación
          />
        </Link>
